# Alejandro — Full-Stack & Data Engineer

> Building systems at the intersection of software, data, and security.

### Full-Stack Engineering · Data Systems · Product Development  
### Security-minded · Systems-oriented · Scaling toward AI & Cloud  

---

## Profile

I design and build scalable systems with a focus on structure, security, and real-world usability.

My work sits at the intersection of:
- Backend architecture
- Data systems
- Product-oriented development

Current direction:
Cloud Infrastructure · DevSecOps · AI Engineering

---

## Featured Work

### GLOSS — Car Detailing Marketplace
Multi-service platform built with a modular architecture.

- Service-based system design
- Frontend + Backend separation
- Scalable structure for marketplace growth
- Docker-based environment (in progress)

---

## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-111111?style=flat&logo=python)
![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=flat&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-111111?style=flat&logo=typescript)
![Java](https://img.shields.io/badge/Java-111111?style=flat&logo=openjdk)
![SQL](https://img.shields.io/badge/SQL-111111?style=flat&logo=postgresql)

### Frameworks & Runtime
![React](https://img.shields.io/badge/React-111111?style=flat&logo=react)
![Next.js](https://img.shields.io/badge/Next.js-111111?style=flat&logo=next.js)
![Node.js](https://img.shields.io/badge/Node.js-111111?style=flat&logo=node.js)
![Express](https://img.shields.io/badge/Express-111111?style=flat&logo=express)
![Astro](https://img.shields.io/badge/Astro-111111?style=flat&logo=astro)

### Data & Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-111111?style=flat&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-111111?style=flat&logo=mongodb)
![MySQL](https://img.shields.io/badge/MySQL-111111?style=flat&logo=mysql)
![Pandas](https://img.shields.io/badge/Pandas-111111?style=flat&logo=pandas)

### DevOps & Tools
![Docker](https://img.shields.io/badge/Docker-111111?style=flat&logo=docker)
![Git](https://img.shields.io/badge/Git-111111?style=flat&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-111111?style=flat&logo=github)
![VSCode](https://img.shields.io/badge/VSCode-111111?style=flat&logo=visualstudiocode)

---

## Engineering Principles

- Modular architecture over monoliths
- Security as a baseline, not an afterthought
- Clean and maintainable code
- Systems thinking over isolated features

---

## Current Focus

- Cloud & Infrastructure (AWS / Azure)
- Backend scalability
- Containerized systems
- Data + AI integration

---

## Contact

GitHub: https://github.com/amndzWRLD